...
export class LineChartComponent implements OnInit {
  ...
  ngOnInit() {
    this._salesDataService.getOrders(1, 100, this.order).subscribe(res => {
      this.allOrders = res['page']['data'];

      this._salesDataService.getOrdersByCustomer(3, this.order).subscribe(cus => {
        ...
        const r = this.getCustomerOrdersByDate(allChartData, dates)['data']; //get unique dates
        console.log('r:', r);
  ...
  getCustomerOrdersByDate(orders: any, dates: any) {
    ...
    const u = Array.from(new Set(prettyDates)).sort(); // get unique set of dates fro
   // console.log(u);
    ...
    customers.reduce((x, y, i) => {
      // console.log('Reducing customer:', y, 'at index:', i);
      ...
      dataSets[i] = {
        ...
        u.reduce((r, e, j) => {
          ...
          // console.log('Reducing date:', e, 'at index:', j, 'customerOrders', customerOrders);
